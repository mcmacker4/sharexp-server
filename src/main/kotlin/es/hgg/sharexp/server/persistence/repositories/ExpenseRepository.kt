...
suspend fun upsertExpense(groupId: UUID, expenseId: UUID?, data: CreateExpenseRequest, computedAmounts: Map<UUID, Long>): UUID? {
    return withContext(Dispatchers.IO) {
        Expenses.upsertReturningId(Expenses.id) {
            if (expenseId != null)
...
suspend fun selectExpensesList(groupId: UUID, principal: UserPrincipal): List<ExpenseListItem> {
    return Expenses.select(Expenses.id, Expenses.title, Expenses.paidBy).where {
        (Expenses.groupId eq groupId) and groupIsVisibleByUser(principal.userId)
    }.map {
        ExpenseListItem(it[Expenses.id], it[Expenses.title], it[Expenses.paidBy])
    }.toList()
}
