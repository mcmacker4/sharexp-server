...
class ExpenseRepository {
    ...
    suspend fun upsertExpense(
        ...
        return Expenses.upsertReturningId(Expenses.id, onUpdateExclude = listOf(Expenses.id, Expenses.createdAt)) {
            if (expenseId != null) it[Expenses.id] = expenseId
    ...
    suspend fun selectExpensesList(
        ...
        return Expenses.select(Expenses.id, Expenses.title, Expenses.paidBy)
            .where { (Expenses.groupId eq groupId) and groupIsVisibleByUser(principal.userId) }
            .page(request) { it.toSortColumn() }
            .map { ExpenseListItem(it[Expenses.id], it[Expenses.title], it[Expenses.paidBy]) }.toList()
    }
    ...
    suspend fun selectExpense(groupId: Uuid, expenseId: Uuid, principal: UserPrincipal): ExpenseInfo? {
        return Expenses
            ...
            .singleOrNull()
            ?.let {
                val participants = selectExpenseParticipants(groupId, expenseId)
    ...
    private suspend fun selectExpenseParticipants(groupId: Uuid, expenseId: Uuid): Map<Uuid, Long> {
        return ExpenseParticipants
            .select(ExpenseParticipants.memberId, ExpenseParticipants.configuredAmount)
            .where {
                (ExpenseParticipants.groupId eq groupId) and (ExpenseParticipants.expenseId eq expenseId)
            }
            .associate { it[ExpenseParticipants.memberId] to it[ExpenseParticipants.configuredAmount] }
    }
